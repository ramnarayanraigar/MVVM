...
class GetStackOverflowDataFromDbUseCase(private val stackOverflowQuestionRepository: StackOverflowQuestionRepository) :
    ...
    override suspend fun execute(input: String): Result<StackOverflowQuestionListData> {
        return when (val result =
            stackOverflowQuestionRepository.getAllData()) {

            is Result.Success -> {
                val chapterListType = typeOf<StackOverflowQuestionListData>().javaType
                val chapterList = Gson().fromJson(result.data[0].all_data, chapterListType) as StackOverflowQuestionListData
